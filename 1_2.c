        ...
        while(client) {
            ...
            if (FD_ISSET(client->socket, &reads) && !client->server_socket && !client->is_server) {
                            ...
                            if (strncmp("GET ", client->request, 4) == 0){
                                ...
                                if (!end_path) {
                                    ...
                                    *end_path = 0;
                                    ...
                                    if (!SSL_CTX_use_certificate_file(client->ctx, "server.crt" , SSL_FILETYPE_PEM)
                                    || !SSL_CTX_use_PrivateKey_file(client->ctx, "server.key", SSL_FILETYPE_PEM)) {
                                        fprintf(stderr, "SSL_CTX_use_certificate_file() failed.\n");
                                        ERR_print_errors_fp(stderr);
                                        return 1;
                                    }
                                    int ret = serve_https_resource(&client_list, client, path);
                                    if (ret < 1) {
                                        if (ret == 0) {
                                            client = next->next;
                                            continue;
                ...
                if(client->is_https) {
                    ...
                    int ret = proxy_https_get_from_client( &client_list, client, get_client(&client_list, client->server_socket));
                    if(ret < 1) {
